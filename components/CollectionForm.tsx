...
export const CollectionForm: React.FC<CollectionFormProps> = ({ isOpen, onClose, onSave, collection }) => {
  ...
  const handleAutoPopulate = async () => {
    ...
    if (images.length > 0 && !window.confirm("This will replace all existing images in this collection. Are you sure?")) {
        return;
    }

    setIsAutoPopulating(true);
    setImages([]); // Clear existing images immediately for better feedback

    try {
        const ai = new GoogleGenAI({ apiKey: process.env.API_KEY });
        
        // Determine number of images based on complexity
        const numImages = 4 + Math.floor((complexity - 1) / 2) * 2; // 1-2 -> 4, 3-4 -> 6, 5 -> 8

        // Step 1: Generate item names
        const listResponse = await ai.models.generateContent({
           model: "gemini-2.5-flash",
           contents: `For a toddler's learning game collection named "${name}" with complexity ${complexity}/5, generate a list of ${numImages} simple, distinct, and easily recognizable items.`,
           config: {
             ...
             responseSchema: {
                ...
                properties: {
                  items: {
                    ...
                    items: {
                      type: Type.STRING,
                      description: 'A single item name, e.g., "Apple" or "Car".'
                    },
                  },
                },
                required: ["items"],
        ...
        const responseJson = JSON.parse(listResponse.text);
        const itemNames: string[] = responseJson.items;
        if (!Array.isArray(itemNames) || itemNames.length === 0) {
            throw new Error("AI did not return a valid list of items.");
        }

        // Step 2: Generate images for each item name, with progress
        const newImages: ImageRecord[] = [];
        for (const itemName of itemNames) {
            const imageResponse = await ai.models.generateImages({
                model: 'imagen-4.0-generate-001',
                prompt: `A bright, simple, and friendly cartoon of a ${itemName}, on a plain white background. Style: vector illustration, for a toddler's learning game.`,
                config: {
                    numberOfImages: 1,
                    outputMimeType: 'image/jpeg',
                    aspectRatio: '1:1',
            ...
            setImages([...newImages]); // Update state progressively
        }
    } catch (error) {
        console.error("Auto-population failed:", error);
        alert(`Sorry, we couldn't automatically create the collection. ${(error as Error).message}`);
    } finally {
        setIsAutoPopulating(false);
    }
  };
      ...
      <div className="bg-white rounded-lg shadow-2xl w-full max-w-2xl max-h-[90vh] flex flex-col">
        ...
        <form onSubmit={handleSubmit} className="flex-grow overflow-y-auto">
          <div className="p-6 space-y-6">
            <div>
              <div className="flex justify-between items-center mb-1">
                ...
                <button
                  ...
                  disabled={isAutoPopulating || !name.trim()}
                  className="flex items-center px-2 py-1 text-sm text-purple-600 hover:text-purple-800 disabled:text-gray-400 disabled:cursor-not-allowed font-medium rounded-md hover:bg-purple-50 transition-colors"
                  title="Generate images based on collection name"
                >
            ...
            <div>
              ...
              <div className="flex space-x-1">
                {[...Array(5)].map((_, i) => (
                  <button type="button" key={i} onClick={() => setComplexity(i + 1)} className="focus:outline-none" disabled={isAutoPopulating}>
            ...
            <div className="relative">
              {isAutoPopulating && (
                <div className="absolute inset-0 bg-white bg-opacity-80 z-10 flex flex-col items-center justify-center rounded-md border-2 border-dashed border-purple-300">
                  <div className="animate-spin rounded-full h-10 w-10 border-t-2 border-b-2 border-purple-500"></div>
                  <p className="mt-4 text-gray-700 font-semibold">Generating collection with AI...</p>
                  <p className="mt-1 text-sm text-gray-500">Please wait, this may take a moment.</p>
                </div>
              )}
              <div className={isAutoPopulating ? 'pointer-events-none opacity-50' : ''}>
                <p className="block text-sm font-medium text-gray-700 mb-2">Images ({images.length})</p>
                <div className={`grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4 ${isAutoPopulating ? 'min-h-[200px]' : ''}`}>
                  {images.map(image => (
          ...
          <div className="flex justify-end items-center p-4 bg-gray-50 border-t">
            ...
            <button type="submit" disabled={name.trim() === '' || images.length < 2 || isAutoPopulating} className="px-6 py-2 bg-blue-500 text-white rounded-md shadow hover:bg-blue-600 disabled:bg-gray-400 disabled:cursor-not-allowed">
              Save Collection
            </button>
          </div>
        </form>
